...
dependencies {
	//Project Modules
	implementation(project(":exchange-server"))
...
tasks.register<Tar>("buildAndPackage") {
	...
	from(layout.projectDirectory.dir("exchange-deploy/src/main/java/com/herron/exchange/exchange/deploy/scripts")) {
		exclude("**/*.md")
	}
	...
	doLast {
		ssh.run(delegateClosureOf<org.hidetake.groovy.ssh.core.RunHandler> {
			session(remotes, delegateClosureOf<org.hidetake.groovy.ssh.session.SessionHandler> {
				...
				execute("tar -xf /home/herron/deploy/exchange-${version}.tar.gz --directory /home/herron/deploy/")
				execute("rm /home/herron/deploy/exchange-${version}.tar.gz ")
